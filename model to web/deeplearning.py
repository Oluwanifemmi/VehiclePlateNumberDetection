...
def object_detection(path, filename):
    ...
    cv2.imwrite(
        '/static/upload/predict/{}'.format(filename), image_bgr)
    return coords


def save_text(filename, text):
    name, ext = os.path.splitext(filename)
    with open('/static/upload/predict/{}.txt'.format(name), mode='w') as f:
        f.write(text)
    f.close()
...
def OCR(path, filename):
    ...
    cv2.imwrite(
        '/static/upload/predict/roi{}'.format(filename), roi_bgr)
